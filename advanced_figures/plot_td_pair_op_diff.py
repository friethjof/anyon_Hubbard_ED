...
def make_plot():
    ...
    if fix_U_or_theta == 'theta':
        ...
        par_list = U_list
        fig_name = f'pair_op_diff_{bc_name}_L_{L}_N_{N}_thpi_{theta_}_{psi0_str}.pdf'
    elif fix_U_or_theta == 'U':
        U_ = f'{U:.1f}'.replace('.', '_')
        par_list = theta_list
        fig_name = f'pair_op_diff_{bc_name}_L_{L}_N_{N}_U_{U_}_{psi0_str}.pdf'
    else:
    ...
    for i, par in enumerate(par_list):
        ...
        pair_op = prop_class.pair_operator()
        ...
        pair_op_left = prop_class.pair_operator()

        prop_class = Propagation(
            **prop_dict,
            psi0_str='psi0_nstate_0-0-0-0-0-0-0-2'
        )
        pair_op_right = prop_class.pair_operator()


        pair_op_diff = pair_op - pair_op_left - pair_op_right

        #-----------------------------------------------------------------------



        l1, = ax.plot(prop_class.time, pair_op_diff)
        line_list.append(l1)
