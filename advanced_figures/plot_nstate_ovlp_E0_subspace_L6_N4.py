...
def make_plot():
    ...
    U = 0.0
    U_ = f'{U:.1f}'.replace('.', '_')
    fig_name = f'nstate_ovlp_E0_subspace_L{L}_N{N}_U_{U_}.pdf'
    ...
    for theta in [0.0, 0.2, 0.5, 0.8, 1.0]:
        ...
        ovlp_lol.append(np.array(nstate_population))
    ...
    fig = plt.figure(figsize=(8.2*1.5, 8.2*0.5), dpi=300)
    fig.subplots_adjust(bottom=0.18, left=0.05, right=0.97, top=0.9)
    ...
    ax1.set_ylabel(r'$\langle \vec{n}_i | \Phi_{E=0} \rangle$', fontsize=12, labelpad=5)

    ax1.set_xticks(i_range)
    ax1.set_xticklabels(xticks_lab, rotation=90, ha='right', fontsize=8,
    ...
    ax1.set_xlim(i_range[0]-1, i_range[-1]+1)
